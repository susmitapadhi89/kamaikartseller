...
export const SellerOrdersList = () => {
  ...
  // Get orders from Redux store
  const { orders, orderloading, ordererror, OrderPagination } = useSelector(
    (state) => state.OrderOpration
  );
  ...
  const orderColumns = [
    {
      ...
      render: (order) => (
        // <div className="min-w-[120px]">
        <div className="sm:min-w-[100px] break-all">
          <div className="font-semibold text-gray-900">{order.orderNumber}</div>
        </div>
      ),
    },
  ...
  useEffect(() => {
    ...
    dispatch(GetSellerOrders(params));
  }, [dispatch, currentPage, debouncedSearch, filters, itemsPerPage]);

  // 🕒 Debounce only searchTerm typing
  useEffect(() => {
  ...
  const updateFilter = (key, value) => {
    ...
    if (key !== "searchTerm") setCurrentPage(1);
  };
  const resetFilters = () => {
    setFilters({
      orderStatus: "",
    ...
    <div className="container mx-auto px-3 py-6">
        ...
        <div className="grid grid-cols-3 gap-4 w-full sm:w-auto">
          <div className="bg-white p-4 rounded-lg shadow-sm border text-center">
            <div className="text-xl font-bold text-blue-600">
              {OrderPagination?.totalOrders || 0}
            </div>
            <div className="text-sm text-gray-600">Total Orders</div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border text-center">
            <div className="text-xl font-bold text-orange-600">{159}</div>
            <div className="text-sm text-gray-600">Pending</div>
          </div>
          <div className="bg-white p-4 rounded-lg shadow-sm border text-center">
            <div className="text-xl font-bold text-green-600">₹{269535}</div>
      ...
      <div className="bg-white p-4 rounded-lg shadow-md border mb-4">
        <div className="flex flex-col md:flex-row justify-between gap-3">
          ...
          <button
            className="flex items-center justify-center border border-gray-300 px-4 py-2 rounded-lg text-gray-700 hover:bg-gray-100 transition"
            onClick={() => setShowFilters(!showFilters)}
          >
